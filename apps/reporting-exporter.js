  ...
  _transform(chunk, encoding, callback) {
    try {
      callback(
        ...
        JSON.stringify(
          this.report.columns
            .map(col => get(JSON.parse(chunk.toString()), col, '')),
        ),
      );
    } catch (e) {
      callback(e);
...
const executeReport = (report, connection, sheets) => new Promise((resolve, reject) => {
...
module.exports.handler = async () => {
  ...
  await Promise.all(REPORTS[process.env.ENVIRONMENT].map(report => executeReport(report, connection, sheets)));
}
