...
describe('validate', () => {
  ...
  test('when the token is invalid', () => {
    expect(() => csrf.validate('not-a-valid-token')).toThrow(CSRFValidationError);
  });
  ...
  describe('an invalid request', () => {
    ...
    test('stops a request with no header', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        "[xsrf][error] invalid token on request to /test-url: Cannot read property 'xsrf-token' of undefined"
      )
    });
